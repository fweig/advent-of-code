...
void repair_manual(const ruleset &rules, manual_t &manual) {
    for (size_t i = 0; i < manual.size(); i++) {
        for (size_t j = 0; j < i; j++) {
            int &x = manual[i];
            int &y = manual[j];
            ...
            if (rules.required_before(x, y)) {
                ...
                if (manual_valid(rules, manual)) {
                    trace("repaired: {}", manual);
                    return;
                }
            }
        }
...
int64_t task(std::string_view input) {
    ...
    auto not_valid = [&](const manual_t &m) {
        return !manual_valid(rules, m);
    };

    auto invalid_manuals = std::ranges::to<std::vector>(
        manuals
        | std::views::filter(not_valid));

    auto repair = [&](manual_t &m) { repair_manual(rules, m); };
    std::ranges::for_each(invalid_manuals, repair);

    auto middle_entries = std::views::transform(invalid_manuals, middle_entry);
