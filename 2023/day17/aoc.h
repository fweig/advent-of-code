...
using iarr2d = arr2d<i64>;
using iarr4d = arr4d<i64>;
using cfv = fv<char>;
...
bool inside(i64 d0, i64 d1, i64 d2, i64 d3, i64 dim[4])
{
  return d0 >= 0 && d1 >= 0 && d2 >= 0 && d3 >= 0 && d0 < dim[0] &&
         d1 < dim[1] && d2 < dim[2] && d3 < dim[3];
}
...
T_ struct arr4d {
  ...
  arr4d(i64 d0, i64 d1, i64 d2, i64 d3)
      : dim(d0, d1, d2, d3)
  {
    data = (T *)ymalloc(d0 * d1 * d2 * d3 * vsize);
  }

  T &operator()(i64 d0, i64 d1, i64 d2, i64 d3)
  {
    if (!inside(d0, d1, d2, d3, dim))
      abort();
    return data[d3 * dim[2] * dim[1] * dim[0] + d2 * dim[1] * dim[0] +
                d1 * dim[0] + d0];
  }
};

T_ i64 width(arr4d<T> a) { return a.dim[0]; }
T_ i64 height(arr4d<T> a) { return a.dim[1]; }
...
T_ struct arr2d {
  ...
  {
    if (!inside(i, dim))
      exit_error("Out of range: i.x = %ld, i.y = %ld, dim.x = %ld, dim.y = %ld", i.x, i.y, dim.x, dim.y);
    return data[index2d(i, dim)];
  }

  T &operator()(i64 x, i64 y) { return this->operator()(idx2{ x, y }); }
...
T_ T *begin(arr2d<T> a) { return a.data; }
T_ T *end(arr2d<T> a) { return a.data + len(a); }
...
T_ void fill(arr4d<T> a, T v)
{
  for (i64 i = 0; i < a.dim[0] * a.dim[1] * a.dim[2] * a.dim[3]; i++) {
    new (&a.data[i]) T{ v };
  }
}
...
#define map2d(iarr, otype, block)                                              \
  ({                                                                           \
    arr2d<otype> oarr{ width(iarr), height(iarr) };                            \
    foreach2d(iarr) {                                                          \
      auto *it = &iarr(_i);                                                    \
      otype val = (block);                                                     \
      oarr(_i) = val;                                                          \
